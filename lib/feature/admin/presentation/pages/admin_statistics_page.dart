...
import 'package:rebtal/core/utils/helper/firestore_dynamic_parsing.dart';
...
class _AdminStatisticsPageState extends State<AdminStatisticsPage> {
  ...
  Widget _buildContent(
    ...
    AdminDataLoaded state,
    bool isDark,
  ) {
    // Users Data
    int totalUsers =
        state.users.length + state.owners.length + state.admins.length;
    Map<int, int> usersByDay = {};
    for (var doc in [...state.users, ...state.owners, ...state.admins]) {
      final timestamp =
          firestoreDynamicToTimestamp(doc.data()['createdAt']);
    ...
    for (var doc in state.chalets) {
      ...
      final timestamp = firestoreDynamicToTimestamp(data['createdAt']);
      final day = timestamp.toDate().day;
      chaletsByDay[day] = (chaletsByDay[day] ?? 0) + 1;
    }
    int totalChalets = state.chalets.length;
    Map<String, int> chaletStatusDist = {
      'Active': activeChalets,
      'Pending': pendingChalets,
    };

    // Bookings Data
    int totalBookings = state.bookings.length;
    double revenue = 0;
    Map<String, int> bookingStatusDist = {
      'Completed': 0,
      'Accepted': 0,
      'Pending': 0,
      'Cancelled': 0,
    ...
    for (var doc in state.bookings) {
      ...
      if (status == 'completed')
        bookingStatusDist['Completed'] =
            (bookingStatusDist['Completed'] ?? 0) + 1;
      else if (status == 'accepted' || status == 'confirmed')
        bookingStatusDist['Accepted'] =
            (bookingStatusDist['Accepted'] ?? 0) + 1;
      else if (status == 'cancelled' || status == 'refused')
        bookingStatusDist['Cancelled'] =
            (bookingStatusDist['Cancelled'] ?? 0) + 1;
      else
        bookingStatusDist['Pending'] = (bookingStatusDist['Pending'] ?? 0) + 1;
        ...
        child: Column(
          ...
          children: [
            ...
            AdminStatisticsLineChart(
              ...
              revenueSpots: _generateSpots(
                revenueByDay.map(
                  (k, v) => MapEntry(k, (v / revenueScaleFactor).toInt()),
                ),
              ),
              revenueScaleFactor: revenueScaleFactor,
              onToggleLegend: (key) => setState(
                () => _chartVisibility[key] = !_chartVisibility[key]!,
              ),
            ),
            const SizedBox(height: 32),
  ...
  List<FlSpot> _generateSpots(Map<int, int> dataMap) {
    if (dataMap.isEmpty)
      return List.generate(31, (i) => FlSpot((i + 1).toDouble(), 0));
    List<FlSpot> spots = [];
    for (int i = 1; i <= 31; i++) {
